...
from shared import N_LOOMS_PER_STIMULUS, DATA_DIRECTORY

from scipy.signal import medfilt

import load.load_track_data as data
...
mouse_ids = {
    "naive": ["1114085", "1114086", "1114087", "1114088", "1114089"],
    "lse": ["1114186", "1114188", "1114189", "1114307", "1114308"],
}
...
HEAD_DIRECTION_DIR = DATA_DIRECTORY / "head_direction"


def track_h5_f_name(_mouse_id):
    d_name = os.path.join(HEAD_DIRECTION_DIR, _mouse_id)
    path = pathlib.Path(d_name)
    f_name = str(list(path.glob("*filtered.h5"))[0])
    return f_name


def loom_starts_npy(_mouse_id):
    f_name = os.path.join(HEAD_DIRECTION_DIR, _mouse_id, "loom_starts.npy")
    return f_name


def get_loom_starts(_mouse_id):
    f_name = loom_starts_npy(_mouse_id)
    _loom_starts = numpy.load(f_name)
    return _loom_starts


def get_tracks(_mouse_id):
    f_name = track_h5_f_name(_mouse_id)
    print(f_name)
...
def transform(this_track):
    ...
    val = numpy.transpose(
        numpy.array([50 * (xs - x0) / (xs - xe), 20 * (y0 - p1[1]) / (p2[1] - p1[1])])
    )
...
def unbound_angles(theta_deg):
    ...
    idx_sign_change = numpy.concatenate(
        (
            numpy.array([False]),
            (numpy.sign(theta_deg[1:]) != numpy.sign(theta_deg[0:-1])),
        )
    )
    ...
    delta_angle[large_positive_sign_changes] = -(
        180 - theta_deg[large_positive_sign_changes]
    ) + -(180 + theta_deg[large_positive_sign_changes - 1])
    delta_angle[large_negative_sign_changes] = (
        180 + theta_deg[large_negative_sign_changes]
...
def match_lse_trials(tracks_naive, tracks_lse):

    assert tracks_naive.shape[1] == tracks_lse.shape[1]

    # this is a random list from matlab (>> rng(1); >> randperm(15)) which implemented this code originally
    random_pairing = (
        numpy.array([3, 15, 6, 5, 7, 13, 4, 8, 9, 1, 11, 10, 12, 2, 14]) - 1
    )

    n_tracks_ = tracks_naive.shape[1]
    print("Matching each lse track with one naive track")
    last_idx = numpy.argmax(tracks_naive, axis=0)

    for ii in range(n_tracks_):

        idx_ = random_pairing[ii]
        tracks_lse[last_idx[idx_] :, ii] = numpy.nan
...
def get_head_angle(head_track, body_track):
    ...
    ha_no_sign = (180 / math.pi) * numpy.arccos(
        shelter_dot_prod / (norm_head_vector * norm_shelter_vector)
    )
...
for group_id in group_ids:
    for i, mouse_id in enumerate(mouse_ids[group_id]):
        ...
        mouse_tracks = get_tracks(mouse_id)
        loom_starts = get_loom_starts(mouse_id)
        ...
        for s in loom_starts:
            ...
            valid_idx = (mouse_tracks["body"][frame_idx, 0] > position_threshold) | (
                timebase < 0
            )

            body = numpy.transpose(
                numpy.array(
                    [
                        medfilt(mouse_tracks["body"][:, 0], 3),
                        medfilt(mouse_tracks["body"][:, 1], 3),
                    ]
                )
            )
            L_ear = numpy.transpose(
                numpy.array(
                    [
                        medfilt(mouse_tracks["L_ear"][:, 0], 3),
                        medfilt(mouse_tracks["L_ear"][:, 1], 3),
                    ]
                )
            )
            R_ear = numpy.transpose(
                numpy.array(
                    [
                        medfilt(mouse_tracks["R_ear"][:, 0], 3),
...
for group_id in group_ids:
    ...
    for i, track in enumerate(body_tracks[group_id]):
        ...
        (
            head_angle_no_sign[group_id][:, i],
            head_angle_with_sign[group_id][:, i],
        ) = get_head_angle(head, body)
    ...
    if group_id == "naive":
        ...
        head_angle_no_sign["lse"] = match_lse_trials(
            head_angle_no_sign["naive"], head_angle_no_sign["lse"]
        )
        head_angle_with_sign["lse"] = match_lse_trials(
            head_angle_with_sign["naive"], head_angle_with_sign["lse"]
        )
    ...
    for i, track in enumerate(body_tracks[group_id]):
        ...
        pre_loom_angle[group_id].append(
            numpy.mean(
                head_angle_with_sign[group_id][zero_frame_idx + numpy.arange(-20, 0), i]
            )
        )

        nan_idx = numpy.where(numpy.isnan(head_angle_no_sign[group_id][:, i]))[0]
        first_nan_idx = (
            nan_idx[0] if len(nan_idx) > 0 else len(head_angle_no_sign[group_id][:, i])
        )
        imax = numpy.argmin(
            head_angle_no_sign[group_id][zero_frame_idx:first_nan_idx, i]
        )

        # take angle most closely directed to the shelter
        post_loom_angle[group_id].append(
            head_angle_with_sign[group_id][zero_frame_idx + imax, i]
        )
