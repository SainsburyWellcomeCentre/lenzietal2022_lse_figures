...
def fig_2c_data():
    ...
    for group_id in group_ids:
        ...
        speed_avg[group_id] = -np.nanmean(np.stack(speeds), axis=0)[
            timebase_to_show
        ]
        speed_std[group_id] = -np.nanstd(np.stack(speeds), axis=0)[
            timebase_to_show
        ]
...
def plot_fig_2c(fig=None, axis=None):
    ...
    ctl_up = np.nansum(
        (speed_avg["ih_ivc_7day"], speed_std["ih_ivc_7day"]), axis=0
    )
    ctl_down = np.nansum(
        (speed_avg["ih_ivc_7day"], -speed_std["ih_ivc_7day"]), axis=0
    )
    ...
    for loom in LOOM_ONSETS_S:
        plt.plot(
            [loom, loom], yl, "k", dashes=default_dash_size, linewidth=0.5
        )
...
def plot_fig_2d(group_id):
    ...
    for i in range(len(theta_before)):
        ...
        if (
            hd_data.response_type[group_id][i] == 2
        ):  # don't do anything for freezes
...
def plot_fig_2e(fig=None, axis=None):
    ...
    for group_id in group_ids:
        ...
        prc_no_response = (
            100 * float(n_no_responses) / total_n_trials[group_id]
        )
...
def plot_fig_2f(fig=None, axis=None):
    ...
    plt.scatter(
        ...
        clip_on=False,
        facecolors="none",
        edgecolors=default_colors["lsie"],
    )
...
def print_stats_fig_2(data_obj):
    ...
    baseline_frames = np.logical_and(
        track_timebase >= baseline_limits[0],
        track_timebase < baseline_limits[1],
    )
    response_frames = np.logical_and(
        track_timebase >= response_limits[0],
    ...
    ih_ivc_7day_baseline = [
        max_min(x[baseline_frames]) for x in data_obj.ih_ivc_7day_speeds
    ]
    ih_ivc_7day_response = [
        max_min(x[response_frames]) for x in data_obj.ih_ivc_7day_speeds
    ]
    ...
    _, ih_ivc_7day_vs_lsie_p_val = ranksums(
        lsie_response, ih_ivc_7day_response
    )

    print(
        f"LSIE baseline vs response: "
    ...
    print(
        f"LSIE response vs IH IVC 7day response: "
    ...
    _, p = fisher_exact(
        [
            [n_freezes[0], n_freezes[1]],
            [
                total_n_trials[0] - n_freezes[0],
                total_n_trials[1] - n_freezes[1],
            ],
        ]
    )

    print(
        f"LSIE n freezes ({n_freezes[1]:.0f}/{total_n_trials[1]:.0f}) vs "
