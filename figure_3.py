...
from shared import a4figure, \
    ...
    format_general_axis, \
    default_colors, \
    create_panel_if_needed, add_figure_labels, add_axes
...
def fig_3_bar_data(dfs):
    ...
    for i in range(len(n_flees)):
        for j in range(i + 1, len(n_flees)):
            _, p_val[i, j] = fisher_exact([[n_flees[i], n_flees[j]],
...
def plot_fig_3d(fig=None, axis=None):
    ...
    ax.text(0.5, 140, 'Pre-test', color='k',
            horizontalalignment='center', verticalalignment='center', fontsize=10)
    ax.text(0, -10, '< 0.2 hrs', color=default_colors['pre_test_immediate'],
            horizontalalignment='center', verticalalignment='top', fontsize=9, rotation=60)
...
def plot_fig_3e(fig=None, axis=None):
    ...
    ax.text(1, 140, 'Post-test', color='k', horizontalalignment='center', verticalalignment='center', fontsize=10)
    ax.text(0, -10, '< 0.2 hrs', color=default_colors['pre_test_immediate'],
            horizontalalignment='center', verticalalignment='top', fontsize=9, rotation=60)
    ax.text(1, -10, '> 24 hrs', color=default_colors['pre_test_24hr'],
            horizontalalignment='center', verticalalignment='top', fontsize=9, rotation=60)
...
def plot_fig_3_bars(ax, n_flees, total_n_trials, p_val, colors, bars_empty=False):
    ...
    for i in range(len(prc_flee)):
        if prc_flee[i] < 30:
            ax.text(i, prc_flee[i] + 5, f'{n_flees[i]}/{total_n_trials[i]}', horizontalalignment='center',
                    verticalalignment='bottom', color='k', fontsize=8, rotation=90)
        else:
            ax.text(i, prc_flee[i] - 3, f'{n_flees[i]}/{total_n_trials[i]}', horizontalalignment='center',
    ...
    for i in range(len(prc_flee)):
        for j in range(i+1, len(prc_flee)):
            if p_val[i, j] < 0.05:
                plt.plot([i, j], 2*[max(prc_flee) + (j-i)*18],
                ...
                if p_val[i, j] > 0.01:
                    ...
                    p_str = '***'
                ax.text((i+j)/2, max(prc_flee) + (j-i)*18, p_str, color='k', horizontalalignment='center',
