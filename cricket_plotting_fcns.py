...
from load.load_cricket_data import (
    body_tracks,
    cricket_tracks,
    reaches_shelter,
    event_times,
...
sigma = 7
n_examples = {"naive": 6, "lse": 5}
example_body_tracks = {"naive": [], "lse": []}
example_cricket_tracks = {"naive": [], "lse": []}
...
def plot_example_tracks(h_ax, group_name):
    ...
    col = (
        default_colors["ih_ivc_7day"]
        if group_name == "naive"
        else default_colors["lse"]
    )
    string = "Naive" if group_name == "naive" else "LSE"
    ...
    [
        h_ax.plot(
            gaussian_filter1d(0.4 - x[1], sigma),
            gaussian_filter1d(x[0], sigma),
            color=col,
            linewidth=0.75,
        )
        for x in b_tracks
...
def plot_histogram(h_ax, group_name, underlying_histogram="bouts_to_shelter"):
    ...
    count_reaching_shelter, bin_edges = numpy.histogram(
        events_reaching_shelter, bins=int(60 / 4), range=(0, 60)
    )
