...
mouse_ids = {
    ...
    "lsie": ["1114186", "1114188", "1114189", "1114307", "1114308"],
}
...
def track_h5_f_name(_mouse_id, datetime_id):
    d_name = os.path.join(
        PROCESSED_DATA_DIRECTORY, _mouse_id, datetime_id, "5_label"
    )
...
def loom_starts_npy(_mouse_id, datetime_id):
    f_name = os.path.join(
        PROCESSED_DATA_DIRECTORY, _mouse_id, datetime_id, "loom_starts.npy"
    )
    return f_name
...
def get_tracks(_mouse_id, datetime_id):
    ...
    for part in body_part_labels:
        t2d = (
            numpy.transpose(numpy.array([df[part]["x"], df[part]["y"]]))
            + dlc_padding
        )
...
def transform(this_track):
    ...
    val = numpy.transpose(
        numpy.array(
            [50 * (xs - x0) / (xs - xe), 20 * (y0 - p1[1]) / (p2[1] - p1[1])]
        )
    )
...
def unbound_angles(theta_deg):
    ...
    large_positive_sign_changes = numpy.where(
        idx_sign_change & idx_positive_val
    )[0]
    large_negative_sign_changes = numpy.where(
        idx_sign_change & idx_negative_val
    )[0]
...
for group_id in group_ids:
    for i, mouse_id in enumerate(mouse_ids[group_id]):
        ...
        for s in loom_starts:
            ...
            frame_idx = s + numpy.arange(
                window_frames[0], window_frames[1] + 1
            )
            valid_idx = (
                mouse_tracks["body"][frame_idx, 0] > position_threshold
            ) | (timebase < 0)
...
for group_id in group_ids:
    ...
    head_angle_with_sign[group_id] = numpy.full(
        (n_frames, n_tracks), numpy.nan
    )
    ...
    if group_id == "naive":
        head_angle_no_sign["naive"] = crop_non_escapes(
            head_angle_no_sign["naive"]
        )
        head_angle_with_sign["naive"] = crop_non_escapes(
            head_angle_with_sign["naive"]
        )
    else:
        head_angle_no_sign["lsie"] = match_lsie_trials(
            head_angle_no_sign["naive"], head_angle_no_sign["lsie"]
        )
        head_angle_with_sign["lsie"] = match_lsie_trials(
            head_angle_with_sign["naive"], head_angle_with_sign["lsie"]
        )
    ...
    for i, track in enumerate(body_tracks[group_id]):
        ...
        pre_loom_angle[group_id].append(
            numpy.mean(
                head_angle_with_sign[group_id][
                    zero_frame_idx + numpy.arange(-20, 0), i
                ]
            )
        )

        nan_idx = numpy.where(numpy.isnan(head_angle_no_sign[group_id][:, i]))[
            0
        ]
        first_nan_idx = (
            nan_idx[0]
            if len(nan_idx) > 0
            else len(head_angle_no_sign[group_id][:, i])
        )
        imax = numpy.argmin(
