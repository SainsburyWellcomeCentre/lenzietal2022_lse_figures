...
DATA_DIRECTORY = pathlib.Path("D:\\SWC\\lenzietal2022_lse\\lenzietal2022_lse_data\\")
...
TRANSFORMED_DF_PATH = DATA_DIRECTORY / "transformed_cricket_paths"

HOUSE_START = 0.2
DLC_PADDING = 40

ARENA_LENGTH_PX = 600.0
ARENA_WIDTH_PX = 240.0
ARENA_SIZE_CM = 50
...
timebase_to_show = np.logical_and(
    track_timebase > track_display_limits[0], track_timebase < track_display_limits[1]
)
time_after_return_to_show = 0.5

GROUP_IDS_FIG1 = ["gh_enriched", "ih_enriched", "ih_ivc_1mth", "ih_ivc_7day"]
N_GROUPS_FIG1 = len(GROUP_IDS_FIG1)

GROUP_IDS_FIG2 = ["ih_ivc_7day", "lse"]
...
def format_track_axis(ax):
    ...
    plt.fill_between(
        ...
        edgecolor=default_colors["shelter"],
    )
...
def x_axis_spots(ax, y_limits, offset=0):
    ...
    [
        plt.scatter(
            x - offset, y_offset, s=scatterball_size(3), facecolor="k", clip_on=False
        )
...
def track_plot_data(dataframe):
    ...
    for i, track in enumerate(dataframe["track"]):
        ...
        if not flees[i]:
            ...
            idx = np.logical_and(
                track_timebase > track_display_limits[0],
...
def plot_tracks_general(t, tracks, linestyle, fig=None, axis=None, color_override=None):
    ...
    flee_color = (
        color_override if color_override is not None else default_colors["flee"]
    )
    freeze_color = (
        color_override if color_override is not None else default_colors["flee"]
    )
    ...
    for i, track in enumerate(tracks):
        if linestyle[i] == "no reaction":
            plt.plot(
                t[i], track, color=default_colors["non_flee"], linewidth=0.8, alpha=0.5
            )
