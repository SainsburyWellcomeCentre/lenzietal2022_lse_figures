...
def fig_1j_data():
    ...
    for group_id in group_ids:
        ...
        dataframe = data.dataframe[group_id]
        dataframe = dataframe[dataframe['is_flee']]
...
def plot_fig_1k(fig=None, axis=None):
    ...
    for i, s in enumerate(avg_speed):
        ...
        if i == ih_ivc_7day_idx:
            plt.scatter(range(5), s, s=scatterball_size(2),
                        facecolors='w', edgecolors=colors[i], linewidths=0.5, zorder=1)
        else:
            plt.scatter(range(5), s, s=scatterball_size(2), facecolors=colors[i], linewidths=0.5)
...
def main():
    ...
    plot_fig_1d_upper(fig=h_fig, axis=axes_dict['b_upper'])
    plot_fig_1d_middle(fig=h_fig, axis=axes_dict['b_lower'])
    plot_fig_1e(fig=h_fig, axis=axes_dict['c'])
    plot_fig_1f(fig=h_fig, axis=axes_dict['d'])
    plot_fig_1k(fig=h_fig, axis=axes_dict['e'])
