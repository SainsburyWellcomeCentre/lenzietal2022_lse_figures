...
def fig_s1a_data():
    ...
    for key, val in mice_by_day.items():
        idx = [
            np.any(a in val) for a in data.dataframe["ih_ivc_7day"]["mouse_id"]
        ]
...
def fig_s1c_data():
    ...
    for df in dfs:
        ...
        robustness_lists.append(
            [robustness_of_mouse(df, x) for x in mouse_ids]
        )
...
def main():
    ...
    plot_fig_s1b_and_s1c(
        speed_lists, stats_s1b, "speed", fig=h_fig, axis=axes_dict["b"]
    )
    plot_fig_s1b_and_s1c(
        robustness_lists,
        stats_s1c,
