...
def fig_1c_data(mouse_id="CA281_1", trial_n=0):
    ...
    track = group.pre_test_trials()[trial_n].track.normalised_x_track[
        0:TRACK_LENGTH
    ]
...
def fig_1e_data():
    ...
    for group_id in group_ids:
        ...
        for i in range(1, 6):
            fraction_on_this_loom = (
                sum(dataframe["last_loom"] == i) / total_n_trials
            )
...
def fig_1h_data():
    ...
    for group_id in group_ids:
        ...
        robustness_lists.append(
            [robustness_of_mouse(dataframe, x) for x in mouse_ids]
        )
...
def plot_fig_1d_lower(fig=None, axis=None):
    ...
    ax.text(
        2.25,
        55,
        "Individually housed, IVC (1 month)",
        fontsize=10,
        horizontalalignment="center",
        verticalalignment="bottom",
    )
...
def plot_fig_1e(fig=None, axis=None):
    ...
    for i, group_id in enumerate(percentage_list):
        ...
        plt.bar(
            len(percentage_list[group_id]) - 0.2 + i * 0.2,
            percentage_list[group_id][-1],
            width=0.2,
            facecolor=default_colors[group_id],
            edgecolor="none",
    ...
    # legend
    plt.fill(
        [1, 1.7, 1.7, 1], [70, 70, 74, 74], color=default_colors["ih_ivc_1mth"]
    )
    plt.fill(
        [1, 1.7, 1.7, 1], [80, 80, 84, 84], color=default_colors["ih_enriched"]
    )
    plt.fill(
        [1, 1.7, 1.7, 1], [90, 90, 94, 94], color=default_colors["gh_enriched"]
    )
    ax.text(
        2,
        72,
...
def plot_fig_1f(fig=None, axis=None):
    ...
    colors = [
        default_colors[x]
        for x in ["gh_enriched", "ih_enriched", "ih_ivc_1mth"]
    ]
...
def plot_fig_1i(fig=None, axis=None):
    ...
    ax.text(
        2.25,
        55,
        "Individually housed, IVC (up to 7 days)",
        fontsize=10,
        horizontalalignment="center",
        verticalalignment="bottom",
    )
    ...
    for j in range(len(percentage_list[:-1])):
        if percentage_list[j] != 0:
            plt.bar(
                ...
                width=0.8,
                facecolor="none",
                edgecolor=default_colors["ih_ivc_7day"],
                clip_on=False,
            )
    plt.bar(
        len(percentage_list),
...
def plot_fig_1k(fig=None, axis=None):
    ...
    colors = [
        default_colors[x]
        for x in ["gh_enriched", "ih_enriched", "ih_ivc_1mth", "ih_ivc_7day"]
    ]
...
def print_stats_fig_1g_and_h(lists, stats, axis_label):
    ...
    for i in range(len(label_str)):
        for j in range(i + 1, len(label_str)):
            ...
            print(
                f"{label_str[i]} (median={median_1:.2f}{uom}, iqr={iqr_1:.2f}{uom}, n={n_1} {n_str}]) "
