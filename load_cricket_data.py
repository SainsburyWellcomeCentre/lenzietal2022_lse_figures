...
paths = {
    "naive": [os.path.join(DF_PATH, x + ".h5") for x in ids["naive"]],
...
def transform_raw_tracks(mid, x_in, y_in):
    ...
    p.estimate(
        np.array([[0, 240], [0, 0], [600, 240], [600, 0]]), new_box_coords
    )
    new_track_x = []
    new_track_y = []
    ...
    for i, path in enumerate(paths[group_id]):
        ...
        if track_type == "raw":
            ...
            if not os.path.isdir(TRANSFORMED_DF_PATH):
                x_body, y_body = transform_raw_tracks(
                    ids[group_id][i],
                    df["body"]["x"] + dlc_padding,
                    df["body"]["y"] + dlc_padding,
                )
                x_cricket, y_cricket = transform_raw_tracks(
                    ids[group_id][i],
                    df["cricket"]["x"] + dlc_padding,
                    df["cricket"]["y"] + dlc_padding,
                )
            else:
                x_body = np.load(
                    os.path.join(
                        TRANSFORMED_DF_PATH, ids[group_id][i], "x_body.npy"
                    )
                )
                y_body = np.load(
                    os.path.join(
                        TRANSFORMED_DF_PATH, ids[group_id][i], "y_body.npy"
                    )
                )
                x_cricket = np.load(
                    os.path.join(
                        TRANSFORMED_DF_PATH, ids[group_id][i], "x_cricket.npy"
                    )
                )
                y_cricket = np.load(
                    os.path.join(
                        TRANSFORMED_DF_PATH, ids[group_id][i], "y_cricket.npy"
                    )
                )

        body = np.array([1 - (x_body / 600.0), (1 - (y_body / 240.0)) * 0.4])
        cricket = np.array(
            [1 - (x_cricket / 600.0), (1 - (y_cricket / 240.0)) * 0.4]
        )
        ...
        bouts = [
            list(group)
            for group in mit.consecutive_groups(list(list(encounters)))
        ]
        ...
        if track_type == "raw":
            ...
            for ii, t in enumerate(these_body_tracks):
                ...
                re_transformed = np.array(
                    [
                        (570 - back_to_raw[0]) / 557,
                        0.4 * (194 - back_to_raw[1]) / (194 - 35),
                    ]
                )
